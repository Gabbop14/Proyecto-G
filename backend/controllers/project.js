  ...
  getProject: function (req, res) {
    ...
    Project.findById(projectId, (err, project) => {
      if (err)
        return res
          .status(500)
          .send({ message: "Error al obtener un proyecto." });
    ...
    Project.findByIdAndUpdate(
      ...
      (err, projectUpdated) => {
        ...
        if (!projectUpdated)
          return res
            .status(404)
            .send({ message: "No se ha encontrado el proyecto." });
  ...
  deleteProject: function (req, res) {
    ...
    Project.findByIdAndDelete(projectId, (err, projectRemoved) => {
      ...
      if (!projectRemoved)
        return res
          .status(404)
          .send({ message: "No se encontró ése proyecto." });
        ...
        Project.findByIdAndUpdate(
          ...
          (err, projectUpdated) => {
            if (err)
              return res
                .status(500)
                .send({ message: "La imagén no se ha subido." });
            ...
            if (!projectUpdated)
              return res
                ...
                .send({
                  message:
                    "No se ha encontrado el proyecto al que asignarle una imagén.",
                });
